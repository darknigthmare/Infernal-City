...
// OpenAI-generated enemy sprites. Collision radii remain gameplay-driven while
// visual sizes preserve each archetype's silhouette and boss hierarchy.
const ENEMY_SPRITE_DATA = {
  swarmer: { src: 'assets/enemies/swarmer.png', size: 42, rotate: true, rotationOffset: -0.62 },
  runner: { src: 'assets/enemies/acid-runner.png', size: 34, rotate: true, rotationOffset: -0.16 },
  brute: { src: 'assets/enemies/cyber-behemoth.png', size: 64, rotate: false },
  vespera: { src: 'assets/enemies/vespera.png', size: 110, rotate: false },
  carmilla: { src: 'assets/enemies/carmilla.png', size: 124, rotate: false },
  hellwarden: { src: 'assets/enemies/hellwarden.png', size: 114, rotate: false },
  leviathan: { src: 'assets/enemies/leviathan.png', size: 188, rotate: true, rotationOffset: 0 }
};
...
class GameEngine {
  ...
  resizeCanvas() {
    ...
    if (this.worldWidth && (scaleX !== 1 || scaleY !== 1)) {
      [this.placedTowers, this.mercenaries, this.petDrones, this.enemies, this.enemyBullets, this.projectiles, this.particles, this.floatingTexts, this.decoys, this.crates, this.powerups, this.hazards]
  ...
  preloadEnemySprites() {
    ...
    Object.entries(ENEMY_SPRITE_DATA).forEach(([type, spriteData]) => {
      ...
      image.addEventListener('error', () => {
        delete this.enemySpriteImages[type];
        console.warn(`Sprite ennemi indisponible : ${spriteData.src}`);
      }, { once: true });
      image.src = spriteData.src;
      this.enemySpriteImages[type] = image;
    });
  }
  ...
  enterAdultExperience() {
    ...
    if (!Object.keys(this.enemySpriteImages).length) this.preloadEnemySprites();
  ...
  drawEnemy(enemy) {
    ...
    const spriteReady = Boolean(sprite && sprite.complete && (sprite.naturalWidth || sprite.width));
    ...
    this.ctx.shadowBlur = enemy.isBoss ? 30 : 8;
    ...
    if (spriteReady) {
      ...
      if (spriteData.rotate) {
        const facingAngle = Number.isFinite(enemy.facingAngle) ? enemy.facingAngle : 0;
        this.ctx.rotate(facingAngle + (spriteData.rotationOffset || 0));
      }
      this.ctx.drawImage(sprite, -visualSize / 2, -visualSize / 2, visualSize, visualSize);
    } else {
